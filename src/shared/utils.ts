...
export const isFn = (value: unknown): value is (...args: any) => any => typeof value === 'function';
...
export const setObjValAtPath = <Obj extends TRecord>(obj: Obj, path: string[], val: unknown): Obj => {
    const key = path.pop();
    if (!key) return obj;
